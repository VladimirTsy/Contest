...
public class Tech {
    ...
    public static void main(String[] args) throws IOException {
        ...
        FibonacciMinPQ<Long> fibonacciMinPQ = new FibonacciMinPQ<>();

        for (int i = 0; i < countTask; i++) {

            fibonacciMinPQ.insert(Long.parseLong(tokenizer2.nextToken()));
        ...
        for(int j = 1; j < countTry ;j++){
           ...
           long k =  fibonacciMinPQ.delMin();
          // while (!fibonacciMinPQ.isEmpty() && fibonacciMinPQ.table.size() > 0 && fibonacciMinPQ.minKey() == k) {
          //     fibonacciMinPQ.delMin();
          // }


            fibonacciMinPQ.insert(k + widthStep);

        }

        System.out.println(fibonacciMinPQ.minKey());

    }


    private static class FibonacciMinPQ<Key> implements Iterable<Key> {
